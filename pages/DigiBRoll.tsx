...
interface DigiBRollProps {
    onNavigate: (view: any) => void;
}

const DigiBRoll: React.FC<DigiBRollProps> = ({ onNavigate }) => {
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        {productImages.map(img => (
                                            <div key={img.id} className="relative aspect-video rounded-xl overflow-hidden border-2 border-indigo-500 group shadow-lg">
                                                ...
                                                <button onClick={() => dispatch({ type: 'REMOVE_PRODUCT_IMAGE', payload: img.id })} className="absolute top-1 right-1 bg-red-500 text-white p-1 rounded-md opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all hover:bg-red-600">
                                                    <X className="w-3 h-3" />
                                                </button>
                                            </div>
                                        ))}
...
const ResultCard: React.FC<{ 
    ...
    return (
        <motion.div 
            ...
            className="bg-[#020617] glass-panel neon-border rounded-2xl sm:rounded-[2.5rem] p-4 sm:p-5 flex flex-col gap-4 shadow-[0_0_30px_rgba(0,212,255,0.05)] group hover:shadow-[0_0_40px_rgba(0,212,255,0.1)] transition-all duration-300"
        >
            ...
            <div className={`relative ${aspectMapping[aspectRatio]} rounded-[2rem] overflow-hidden bg-slate-50 dark:bg-black/40 border border-slate-100 dark:border-white/5 flex items-center justify-center group/img-box shadow-inner`}>
                {result.status === 'loading' ? (
                    ...
                    <>
                        ...
                        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-4 flex items-end justify-center opacity-100 md:opacity-0 md:group-hover/img-box:opacity-100 transition-all duration-500">
                            <button onClick={() => onZoom(result.imageUrl!)} className="w-12 h-12 bg-white/20 hover:bg-white/40 text-white rounded-full backdrop-blur-xl border border-white/20 flex items-center justify-center transition-all shadow-2xl">
            ...
            {result.status === 'done' && (
                <div className="flex flex-col gap-3 pt-2">
                    ...
                    <a href={result.imageUrl!} download={`digi_broll_${idx+1}.png`} className="w-full flex items-center justify-center gap-3 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-[10px] font-black uppercase shadow-lg transition-all active:scale-95 group/dl">
